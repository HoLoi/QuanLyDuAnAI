...
# CẬP NHẬT CHẶN TỰ KHÓA VÀ BẢO VỆ ADMIN CUỐI CÙNG (02/07/2026)

## 1. Phạm vi thay đổi

Đã bổ sung bảo vệ cho thao tác khóa tài khoản mà không thay đổi schema, migration, route, cơ chế phân quyền, workflow kích hoạt email hoặc chức năng quên mật khẩu.
...
`AccountService.AuthenticateAsync` đã phát claim `MaNguoiDung` khi tạo cookie đăng nhập. `NhanSuController` đọc claim này, chuyển thành `int` và:

- đưa `MaNguoiDungHienTai` vào `NhanSuPageViewModel` để giao diện xác định đúng dòng hiện tại;
- truyền `MaNguoiDung` của người thao tác vào `INhanSuService.LockAccountAsync`;
- từ chối gọi service nếu không xác định được tài khoản đang đăng nhập.

Mã người thao tác không được lấy từ form hoặc query string. Razor không truy vấn database và không phát sinh N+1.

## 3. Quy tắc không cho tự khóa

`NhanSuService.LockAccountAsync` so sánh trực tiếp mã người thao tác với mã nhân sự mục tiêu trước khi thay đổi dữ liệu. Nếu trùng nhau, service trả lỗi:

`Bạn không thể tự khóa tài khoản đang đăng nhập.`

Quy tắc nằm ở service nên vẫn có hiệu lực khi người dùng tự tạo request POST thủ công. Controller bắt lỗi theo pattern hiện tại, chỉ đặt `TempData["Error"]`; thông báo thành công chỉ được đặt sau khi service hoàn tất.

Trong bảng nhân sự, dòng có `MaNguoiDung` trùng `MaNguoiDungHienTai` không hiển thị nút `Khóa`, thay bằng badge `Tài khoản hiện tại`. Các nút sửa, xóa, gửi kích hoạt và logic mở khóa hiện có không bị thay đổi.

## 4. Quy tắc Admin cuối cùng đang hoạt động

Trước khi khóa tài khoản thuộc role Admin, service đếm các Admin đang hoạt động bằng truy vấn EF Core kết hợp:

- `AspNetUserRoles` và `AspNetRoles`: role có `NormalizedName = "ADMIN"` hoặc tên role quy về chữ hoa là `ADMIN`;
- `NguoiDung.IsDeleted != true`;
- `Aspnetusers.EmailConfirmed = true`;
- `LockoutEnd` không có giá trị hoặc không còn lớn hơn thời điểm UTC hiện tại.

Nếu tài khoản mục tiêu đang hoạt động và tổng số Admin đang hoạt động không lớn hơn 1, thao tác bị từ chối với thông báo:

`Không thể khóa Admin cuối cùng đang hoạt động của hệ thống.`

Toàn bộ kiểm tra và cập nhật khóa chạy trong transaction `Serializable`, tránh hai yêu cầu khóa Admin đồng thời cùng vượt qua phép kiểm tra. Giới hạn `MaxAdminCount` hiện có vẫn chỉ phục vụ rule tạo/gán role Admin và không được dùng thay cho phép đếm Admin hoạt động.

Các ràng buộc cũ vẫn được gọi: không khóa nhân sự đang quản lý dự án, không khóa Leader team và vẫn ghi cảnh báo khi còn công việc chưa hoàn thành.

## 5. POST, anti-forgery và phiên đăng nhập

Hai action `KhoaTaiKhoan` và `MoKhoaTaiKhoan` tiếp tục dùng `[HttpPost]` và `[ValidateAntiForgeryToken]`. Các form Razor dùng form tag helper nên phát anti-forgery token theo cơ chế MVC; request thiếu hoặc sai token bị middleware MVC từ chối trước khi action thay đổi dữ liệu.

Khi khóa tài khoản khác, service đổi `SecurityStamp` bên cạnh `LockoutEnd`. Tuy nhiên, cấu hình cookie trong `Program.cs` hiện không có `ValidatePrincipal` hoặc middleware đối chiếu `SecurityStamp` theo từng request. Vì vậy thay đổi này chỉ tạo dấu hiệu bảo mật để cơ chế kiểm tra phiên có thể sử dụng; không thể khẳng định cookie đang tồn tại bị đăng xuất ngay. Cookie có thể còn hiệu lực đến khi được kiểm tra lại hoặc hết hạn.

## 6. Kiểm tra

Đã kiểm tra tĩnh trên source:

- dòng hiện tại không đi vào nhánh hiển thị form khóa và có badge `Tài khoản hiện tại`;
- POST tự khóa bị service từ chối trước transaction và trước cập nhật `LockoutEnd`;
- khóa Admin chỉ được phép khi mục tiêu không phải Admin hoạt động cuối cùng;
- Admin bị khóa, chưa kích hoạt hoặc có hồ sơ xóa mềm không được tính là Admin hoạt động;
- tài khoản Manager/Employee khác vẫn đi qua luồng khóa và các ràng buộc cũ;
- rule quản lý dự án và Leader team vẫn còn nguyên;
- action khóa/mở khóa vẫn là POST có anti-forgery;
- nút mở khóa vẫn giữ điều kiện và action cũ.

Workspace không có test project tự động cho module Nhân sự và lượt này không kết nối/chỉnh dữ liệu SQL Server để dựng các trạng thái tài khoản. Vì vậy các kịch bản phụ thuộc dữ liệu thật và trình duyệt chưa được tuyên bố là đã kiểm thử tích hợp runtime.
...
Kết quả: build thành công, `0 Error(s)`, `2 Warning(s)`. Hai warning `CS1998` nằm ở `Services/Implementations/FileTienDoCongViecService.cs` dòng 26 và 31, có sẵn và ngoài phạm vi thay đổi.

Các file sửa đã được kiểm tra bằng UTF-8 decoder nghiêm ngặt: đều là UTF-8 hợp lệ, không phát hiện chuỗi tiếng Việt mới bị mojibake. Trạng thái BOM hiện có của từng file được giữ nguyên.
